...
def translait(get_word):
    ...
    # Записываем в переменые выхода нули, что бы они очишались при каждом вызове
    word = 0
    word_rus = 0
    result_en = 0
    ...
    if len(pronouncing.phones_for_word(result_en)) > 0:  # условие нахождения фонемы
        ...
        for i in cleaned:  # сопоставляем фонемы с словарем русского произношения
        ...
        print("Это невиданная хрень, мы такого слова не учили")
        runglish = 'nope'
...
# Функция поиска слова в рускоязычном словаре по транскрипции ливенштейну
...
def get_mem(mnemonika, word_ru):
    url_photo_mem = ''
    for item in parser.search(f'мем {word_ru} {mnemonika}'):
...
# Функция риспелинга англизского слова в соотвествии с произношением
...
# dict = pd.read_csv('/Users/antonzagorelskii/Desktop/Elbrus/Projects/NeuroMonika_bot/rus_dict_50.csv', sep=';')
